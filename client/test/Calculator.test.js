...
/*Tests that validateCoordinatesOnInput() catches incorrectly formatted input*/
function testCoordinateValidationAndConversion() {
  ...
  const tipConfigRequest = {
    'type'        : 'distance',
    'version'     : 1,
    'origin'      : calculator.state().origin,
    'destination' : calculator.state().destination,
    'earthRadius' : 3959
  };

  let testConfigRequest = calculator.instance().validateAndConvertCoordinates(tipConfigRequest);

  expect(testConfigRequest.origin.latitude).toEqual("0");
  expect(testConfigRequest.origin.longitude).toEqual("1");
  expect(testConfigRequest.destination.latitude).toEqual("2");
  expect(testConfigRequest.destination.longitude).toEqual("3");
}

test('Tests the validation and conversion of coordinates', testCoordinateValidationAndConversion);
